...
const DashboardPage = () => {
  ...
  const [callLanguage, setCallLanguage] = useState<"hi" | "en" | "te">("hi");
  ...
  const phoneDigits = callPhone.replace(/\D/g, "");
  const needsPhone = !phoneDigits || /^0+$/.test(phoneDigits);
  ...
  const handleRequestCall = async () => {
    ...
    try {
      const result = await requestTelebeliCall({ phone: callPhone, language: callLanguage });
      if (result?.error) {
        toast.error(result.error);
        return;
      }
      toast.success(result?.message || "Call queued. You should receive a call shortly.");
      if (result?.phone) setCallPhone(result.phone);
    } catch {
  ...
  useEffect(() => {
    if (dashboardData?.profile?.phone) setCallPhone(dashboardData.profile.phone);
          ...
          <div className="grid grid-cols-1 gap-3 md:gap-5 xl:grid-cols-[minmax(0,1.35fr)_minmax(340px,0.65fr)]">
            ...
            <div className="space-y-3 md:space-y-5">
              ...
              <section className="rounded-3xl border border-slate-200/70 bg-white p-4 shadow-[0_10px_30px_rgba(15,23,42,0.06)] md:p-5">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="text-xs font-black uppercase tracking-[0.1em] text-[#64748B]">Call support</p>
                    <h2 className="mt-1 text-xl font-black text-[#0F172A]">Request Call</h2>
                  </div>
                  <PhoneCall className="h-6 w-6 text-[#C41E3A]" />
                </div>
                <div className="mt-4 space-y-3">
                  <input
                    ...
                    className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-3 py-3 text-sm font-bold text-[#0F172A] outline-none transition focus:border-[#C41E3A] focus:bg-white"
                  />
                  {needsPhone && (
                    <p className="text-xs font-semibold text-amber-700">
                      Add your mobile number before requesting a call.
                    </p>
                  )}
                  <div className="grid grid-cols-3 gap-2">
                    {[
                      ...
                      <button
                        key={item.value}
                        type="button"
                        onClick={() => setCallLanguage(item.value as "hi" | "en" | "te")}
                        className={`rounded-xl px-2 py-2 text-xs font-black transition ${
                          callLanguage === item.value
                  ...
                  <button
                    ...
                    disabled={isRequestingCall || needsPhone}
                  ...
                  >
                    ...
                    {isRequestingCall ? "Requesting..." : "Request Call"}
                  </button>
                </div>
              </section>
