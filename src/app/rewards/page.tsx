...
import { Reward, Recipient, RewardProgram } from "@/types";
import {
  CheckCircle,
  Loader2,
  Gift,
...
const TRAVEL_PROGRAM_NAMES: Record<string, string> = {
  "europe-trip-2026": "Europe Trip 2026",
  "cruise-trip-2026": "Cruise Trip 2026",
};
...
const RewardsPage = () => {
  ...
  const fetchInitialData = useCallback(async () => {
    setDataLoading(true);
    try {
      const [rewardsData, programsData, activeLegsResp] = await Promise.all([
        getRewards(),
        getRewardPrograms(),
        getActiveLegsCount(),
      ]);
  ...
  useEffect(() => {
    ...
    const openClaimsFromHash = () => {
      if (window.location.hash === "#reward-claims" && !dataLoading) {
        setShowDetails(true);
